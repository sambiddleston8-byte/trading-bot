...
|---|---|---|---|
| 1 — foundation | Immutable decision/model-version records, protected Git workflow, transactional local change boundary and architecture/risk audit are implemented and tested. The legacy executable pickle market cache is replaced by immutable, integrity-checked Parquet snapshots that remain explicitly non-authoritative. | Continued migration of remaining legacy paths and authoritative PostgreSQL cutover before unattended operation. | Safe local foundation substantially complete. |
