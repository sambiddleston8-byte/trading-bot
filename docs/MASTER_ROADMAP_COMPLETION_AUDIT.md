...
| 4 — Alpaca paper | Immutable paper-order proposals, deterministic local fills, methodology pre-flight, a live-prohibition checklist and a paper-host-only read-only account adapter exist. An inactive human-preregistered risk-policy ledger records exact order, position, gross-exposure, daily-loss and account-freshness limits without defaults; a one-way local stop follows the hashed paper account/stop identity across replacement policies. A normalized read-only snapshot conservatively combines pinned long positions, pending BUY exposure and a separately sourced previous-close daily-loss basis. Exact long-position and open-order quantity × mark-price evidence reconciles every ticker/order and aggregate to that snapshot, including separate pending BUY and SELL totals. A shadow assessment compares exact proposal notional/projected exposures/loss/freshness against those limits. A SELL-only composite binds both quantity ledgers, calculates held minus pending-SELL shares, rechecks freshness, preserves account/stop-identity latches and fails closed on insufficient quantity without treating pending BUY orders as available. An inactive human-supplied BASE/PESSIMISTIC execution policy and shadow calculator reuse the replay cost floors, apply adverse price movement and configured commission, and keep SELL conservative risk from falling with worse proceeds. | User-created paper account, securely stored credentials, successful account check, human-selected real risk and execution-cost values, real read-only position/order/previous-close collection, authenticated reference/market/volume evidence, broker reconciliation, complete applicable fee/borrow treatment, binding stress evidence into an authenticated enforcement assessment, paper submission/cancel/fill/reconciliation adapter and externally anchored stop evidence before operational reliance. | Explicit broker-account connection, risk-limit and cost decisions required; quantity/stress/shadow records are inactive, reference-price-only, synthetic/unreconciled and unauthenticated; quantity and price results are internal arithmetic rather than broker proof, limits remain unenforced and live endpoint/order submission unsupported. |
