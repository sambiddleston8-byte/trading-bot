...
| 4 — Alpaca paper | Immutable paper-order proposals, deterministic local fills, methodology pre-flight, a live-prohibition checklist and a paper-host-only read-only account adapter exist. An inactive human-preregistered risk-policy ledger records exact order, position, gross-exposure, daily-loss and account-freshness limits without defaults; a one-way local stop follows the hashed paper account/stop identity across replacement policies. A normalized read-only snapshot conservatively combines pinned long positions, pending BUY exposure and a separately sourced previous-close daily-loss basis. Exact long-position and open-order quantity × mark-price evidence reconciles every ticker/order and aggregate to that snapshot, including separate pending BUY and SELL totals. A shadow assessment compares exact proposal notional/projected exposures/loss/freshness against those limits, shares the permanent-stop lock and never treats SELL orders as reducing pre-fill risk. | User-created paper account, securely stored credentials, successful account check, human-selected real policy limits, real read-only position/order/previous-close collection, broker reconciliation/authentication, binding both quantity ledgers into SELL sufficiency, stressed execution price/fees, authenticated enforcement assessment, paper submission/cancel/fill/reconciliation adapter and externally anchored stop evidence before operational reliance. | Explicit broker-account connection and risk-limit decisions required; quantity/shadow records are inactive, reference-price-only, synthetic/unreconciled and unauthenticated; limits remain unenforced and live endpoint/order submission unsupported. |
