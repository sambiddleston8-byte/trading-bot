...
Implementation checkpoint: the provider-neutral PIT security master, buffered-universe reconstruction, and XNYS session/partition research mechanics are complete. Immutable permanent-security-ID events cover listings, ticker changes, index additions/removals and delistings under effective-time and availability-time cutoffs; overlapping ticker assignments fail closed, ticker reuse resolves by permanent identity, and a deterministic delisted-name fixture reproduces exactly. The monthly S&P 500 selection path applies the frozen price and 20-session median-dollar-volume floors, top-100 entry/top-120 retention buffer, 120-name ceiling, SPY exclusion and permanent-ID tie-break without consulting a current constituent list. A provider-neutral append-only calendar/partition ledger now normalizes regular and early-close XNYS sessions under the five-timestamp contract, pins source bytes, records immutable TRAIN, VALIDATION and TEST boundaries, and enforces exactly H + embargo daily decision periods at both seams; synthetic fixtures remain explicitly incomplete, unqualified and unadmitted, with VALIDATION and TEST access disabled. The currently authorized Massive Basic route provides only two years of end-of-day history, so it may support a wider research-only TRAIN campaign but cannot satisfy the unchanged three-year production TRAIN minimum. No provider payload, fresh VALIDATION window or TEST data is admitted by this checkpoint; Stage 1 remains open for qualified production-scale source capture, corporate actions, authoritative calendar coverage and admission.
