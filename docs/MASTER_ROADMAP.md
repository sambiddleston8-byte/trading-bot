...
Implementation checkpoint: the provider-neutral PIT security master, buffered-universe reconstruction, corporate-action research boundary, and XNYS session/partition research mechanics are complete. Immutable permanent-security-ID events cover listings, ticker changes, index additions/removals and delistings under effective-time and availability-time cutoffs; overlapping ticker assignments fail closed, ticker reuse resolves by permanent identity, and a deterministic delisted-name fixture reproduces exactly. The monthly S&P 500 selection path applies the frozen price and 20-session median-dollar-volume floors, top-100 entry/top-120 retention buffer, 120-name ceiling, SPY exclusion and permanent-ID tie-break without consulting a current constituent list. A provider-neutral append-only corporate-action ledger pins source bytes, records splits, cash dividends and terminal outcomes under the five-timestamp contract, links every event to permanent identity and terminal outcomes to the exact delisting record, and materializes only information available by its economic effective time into GuardrailedBacktestEngine inputs; late evidence fails closed. Current-master reconciliation surfaces stale evidence without rewriting its pinned historical prefix, explicit replacements preserve a single immutable supersession chain with an enumerated reason, and only the active leaf may materialize for replay. A provider-neutral append-only calendar/partition ledger normalizes regular and early-close XNYS sessions under the same contract, pins source bytes, records immutable TRAIN, VALIDATION and TEST boundaries, and enforces exactly H + embargo daily decision periods at both seams. Calendar corrections preserve one explicit immutable supersession chain; coverage is exact except for an enumerated overlapping boundary correction, only the active calendar may derive or serve its single partition manifest, and superseded calendars and manifests remain auditable but fail closed at consumption. The provider-neutral append-only daily-bar ledger pins synthetic TRAIN rows to the active calendar, partition manifest, permanent-security-master prefix and five-timestamp contract, preserves explicit correction chains, retains read compatibility for the prior strict schema, and permits only the active leaf on the active calendar to materialize immutable GuardrailedBacktestEngine bars. Its constant-membership rectangle remains available as a narrow fail-closed shape; the interval-capable shape records contiguous per-security session bounds and ticker segments, retains a delisting-day bar when the identity is listed at that session open, materializes permanent security IDs plus PIT S&P 500 add/remove events, and never splices ticker histories. Against its exact pinned master prefix it fails closed when any PIT member lacks its session-open or decision-time bar, and it requires the first still-listed session at which a held removal exit can execute with real liquidity. GuardrailedBacktestEngine consumes changing session symbol sets while requiring an exact bar for every active, held, protective-exit or pending-intent permanent ID; universe removal is an engine constraint that closes a held name at the first open where the removal is known or creates an immutable forced-EXIT intent when it becomes known only at a decision, reductions still execute before increases, terminal settlement supersedes pending protective orders, and late ADD or REMOVE evidence cannot affect eligibility until available. An append-only synthetic TRAIN source-bundle ledger now binds the exact pinned master prefix, active calendar and partition manifest, active interval daily-bar leaf, and the complete microsecond-contiguous, non-overlapping set of active corporate-action leaves covering each permanent ID's own replay interval to reverified content-addressed source bytes; it independently rechecks that every bar lies wholly inside the pinned TRAIN partition, historical bundles remain auditable after a correction, and only current dependencies materialize together into permanent-ID-aligned GuardrailedBacktestEngine inputs. This authenticates the referenced top-level payload bytes only: per-row bar bytes, source semantics and cross-source completeness of any scope remain explicitly unproven. These interval, dynamic-replay and cross-ledger assembly mechanics are complete only on bounded deterministic synthetic fixtures: source coverage remains explicitly unproven, unqualified and unadmitted, with performance/promotion authority false and VALIDATION and TEST access disabled. The currently authorized Massive Basic route provides only two years of end-of-day history, so it may support a wider research-only TRAIN campaign but cannot satisfy the unchanged three-year production TRAIN minimum. A bounded Windows-local Norgate export boundary now records Norgate numerical asset IDs, the requested and resolved symbols, package and database vintage, a license-restricted source hash, and requests `NONE` adjustment, `NONE` padding, daily bars and current-vintage S&P 500 constituent evidence; cross-vintage identity stability and provider semantics remain unverified, and constituent evidence remains supplemental rather than an admitted replay role. Its ingest path stamps the local system clock at file read, retains the bar's economic session date separately from current-vintage availability and cannot authorize performance. Apart from the bounded real checks described below, its mechanics have been verified only with deterministic provider-shaped fixtures. The first check exercised the export, one-shot local transfer and quarantine path once on an AAPL payload for the 2026-08-17 through 2026-08-19 sessions from the `US Equities` database at database update timestamp `2026-08-20T07:37:43.000000+00:00`, using Norgate Data package 1.0.77; the exact 1,757 bytes were verified on write, transferred outside source control, matched SHA-256 `ee58ef4f4ac01175e28654cf8658e195570687dd5885540ae3bbbaae65069500`, and normalized with every authority flag false. A later same-vintage repeat export produced raw SHA-256 `fc9c5eae0480975a67f608da565fed1b2df46cf4a43d5a096267ccba8052606f`; the fail-closed comparison contract proved every invariant field identical after excluding only the deliberately changing `exported_at`, yielding invariant SHA-256 `31e0f59ff78deaad71449315bb56130cc40f69a1e97b8ae33946a4a7ef897830`. This shows only that this single AAPL three-session export reproduced byte-identically outside `exported_at` at one database vintage on one package version; it is not evidence of repeatability at scale, across symbols, across vintages, or of provider semantics. A fail-closed sharded-capture manifest now requires one exact staged fixed-watchlist catalog, derives its asset-ID-ordered expected symbol partition, matches every observed asset ID, symbol and security name plus the database/package vintage, and binds the catalog source, canonical-entry and receipt-evidence hashes alongside immutable shard hashes and unique cross-shard asset/date identities. Ambiguous catalog symbol reuse fails closed. That catalog-bound assembly path is verified only with deterministic provider-shaped fixtures; it establishes content-addressed linkage to one unqualified current-vintage catalog only, not that a real capture is complete, historically available, semantically correct or fit for replay. A separate fixed-watchlist catalog boundary now binds the provider-named `S&P 500 Current & Past` identity list at exactly one current database vintage to sorted stable asset IDs, symbols and security names plus exact entry, source and receipt-evidence hashes, while withholding security-master admission. The real local capture from Norgate Data package 1.0.77 and `US Equities` database update timestamp `2026-08-20T07:37:43.000000+00:00`, exported at `2026-08-20T09:53:06.693069+00:00` and locally received at `2026-08-20T09:56:57.440417+00:00`, contained 543 entries spanning asset IDs 124920 through 4082931 and 44,323 bytes; the exact source SHA-256 is `a93267ef8314889c6ae9dc4e351b19c20c6f6364c31cde75c1512f6b1c4616dd`, the canonical entries SHA-256 is `58060e66b1724b0c2725d42392199783673da5ee0a585e8ba73e34ded44af45c`, the local receipt-evidence SHA-256 is `606d49175827b4627bb703045a57c9410ef770609a5a3a04431450a1bd00148b`, and no cross-asset symbol reuse was surfaced in this one catalog. These hashes establish the byte identity of this one capture only. Neither the 543-entry count nor zero surfaced symbol reuse is an expected-count anchor or evidence of the depth of the watchlist's `Past` component. The provider's watchlist label is not proof of complete historical membership, PIT availability, ticker-history or corporate-action semantics, cross-vintage stability, coverage completeness or fitness for replay, and the license-restricted provider rows remain outside Git. These are short, narrow, current-vintage connectivity and quarantine checks only, not alpha evidence or a promotable sample: no real Norgate row is authenticated, qualified, admitted or performance-authorized, and historical availability, ticker history, correction semantics and coverage remain unproven. No provider payload, fresh VALIDATION window or TEST data is admitted by this checkpoint; Stage 1 remains open for qualified production-scale source capture, authoritative full-history security-master, corporate-action and calendar evidence, cross-source completeness proof, and admission.

The Norgate V2 local export records exactly one content-addressed disposition for every requested stable asset identity: `ROWS_PRESENT` with its exact row count or `NO_ROWS_IN_REQUESTED_WINDOW` with zero rows. `ROWS_PRESENT` asserts only that at least one row fell inside the requested window; it is not a claim of complete or continuous window coverage. A rowless asset remains in the catalog-derived partition and cannot be silently excluded, but neither disposition implies delisting, eligibility, complete provider coverage or any other historical semantic conclusion. V1 exports remain strictly readable. One bounded real TRAIN-only quarantine capture requested `2022-01-03` through `2025-02-28` from Norgate Data package 1.0.77 and the `US Equities` database update timestamp `2026-08-20T07:37:43.000000+00:00`. Its six shards bound all 543 current-catalog identities, recorded 539 with at least one row and four with explicit zero-row dispositions, and contained 71,427 rows in 23,458,025 transferred bytes. Only 133 distinct sessions from `2024-08-19` through `2025-02-28` were observed; positive per-identity row counts had minimum 11, mean 132.52 and maximum 133. Against roughly 793 exchange sessions in the requested interval, the aggregate density is about 17% of the identity-session rectangle. The cause and entitlement boundary of this shortfall remain unqualified. This capture therefore does not evidence the required three-year TRAIN history and cannot support production admission or promotion. The immutable shard SHA-256 values, in catalog partition order, are `1843a7c0a3d82d8638a01cb6bdf889e2b1818e78d504df6dd20dc92a91eafd5f`, `0f8befee240f9093257172b6cce525dc17b375622acac79c459e104186bc79a5`, `5d2652f2a2cdd879f366a4dac4d3d1136b653e780b0994e25d17bc1d934fe80c`, `25423d678b99eac8297f744d41f3c53026036098c579329707c0c5ba6faab598`, `3e14fa786b46d5adbce45fe6f08cbd2509c629b65bdb6c748a6bfd89b9f6b9ba`, and `9c4d19c705cdcd432a7f52c118b1a4672d638c9abd3ecc99493ab0cb71c15509`; the one-shot assembly manifest SHA-256 is `ee11e5170ff797bea9099c7c6be5e0ebc862ed78fb80e7d70f39cdea99e57e58`. Provider rows and dispositions remain license-restricted outside Git. This is a current-vintage quarantine/connectivity result, not evidence of catalog completeness, provider-origin authentication, PIT membership or ticker semantics, correction history, cross-source completeness, replay fitness, incremental alpha or promotion eligibility; every authority flag remains false and neither VALIDATION nor TEST was accessed.

For the sharded-capture checkpoint, the expected symbol partition is now derived from and content-addressed to one exact staged catalog rather than free-typed input. The catalog remains a current-vintage, provider-named, unqualified list: this binding does not authenticate preregistration, provider origin, PIT membership, completeness, historical availability, ticker semantics, observation selection or fitness for replay; `source_bytes_authenticated`, `observation_selection_validated` and every other authority flag remain false.
