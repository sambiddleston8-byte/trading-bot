...
Implementation checkpoint: the provider-neutral PIT security master and buffered-universe reconstruction are complete as research mechanics. Immutable permanent-security-ID events cover listings, ticker changes, index additions/removals and delistings under effective-time and availability-time cutoffs; overlapping ticker assignments fail closed, ticker reuse resolves by permanent identity, and a deterministic delisted-name fixture reproduces exactly. The monthly S&P 500 selection path applies the frozen price and 20-session median-dollar-volume floors, top-100 entry/top-120 retention buffer, 120-name ceiling, SPY exclusion and permanent-ID tie-break without consulting a current constituent list. The currently authorized Massive Basic route provides only two years of end-of-day history, so it may support a wider research-only TRAIN campaign but cannot satisfy the unchanged three-year production TRAIN minimum. No provider payload, fresh VALIDATION window or TEST data is admitted by this checkpoint; Stage 1 remains open for qualified production-scale source capture, corporate actions, calendars, partition seams and admission.
